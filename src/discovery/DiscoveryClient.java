...
public class DiscoveryClient extends Thread {//extends Task {
	...
	public static void discovery() {
		...
		try {
			...
			try {
				DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, InetAddress.getByName("255.255.255.255"), 8888/*DiscoveryThread.port*/);
				c.send(sendPacket);
				System.out.println(DiscoveryClient.class.getName() + ">>> Request packet sent to: 255.255.255.255 (DEFAULT)");
			} catch (Exception e) {
			}
			...
			while (interfaces.hasMoreElements()) {
				...
				for (InterfaceAddress interfaceAddress : networkInterface.getInterfaceAddresses()) {
					...
					System.out.println(
							DiscoveryClient.class.getName() + ">>> Request packet sent to: " + broadcast.getHostAddress() + "; Interface: " + networkInterface.getDisplayName());
				}
			}

			System.out.println(DiscoveryClient.class.getName() + ">>> Done looping over all network interfaces. Now waiting for a reply!");
			while (true) {
				// Wait for a response
				...
				// We have a response
				System.out.println(DiscoveryClient.class.getName() + ">>> Broadcast response from server: " + receivePacket.getAddress().getHostAddress());
